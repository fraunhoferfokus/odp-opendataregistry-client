...
public class ODRClientImpl implements ODRClient {
    ...
    public void init() throws OpenDataRegistryException {
        ...
        try {
            props.load(Thread.currentThread().getContextClassLoader().getResourceAsStream(PROPERTIES_FILENAME));
        } catch (IOException e) {
            log.error("init open data registry client implementation (CKAN)", e);
            throw new OpenDataRegistryException("Could not find or read properies file 'ckan.properties'.");
        }
        init(props);
    }
    ...
    public void init(Properties props) {
        ...
        ClientExecutor executor = new ApacheHttpClient4Executor(new DefaultHttpClient(new ThreadSafeClientConnManager()));
    ...
    public List<Category> listCategories() {
        ...
        try {
            log.trace("REST > calling action api 'group_list' with: {}", ALL_FIELDS);
            long start = System.currentTimeMillis();
            response = action.listGroups(ALL_FIELDS);
            log.debug("/api/3/action/group_list: {}ms", System.currentTimeMillis() - start);
            ...
            if (response.getResponseStatus() == Status.OK) {
                ...
                log.trace("REST < (http) returns: {} - {}", response.getResponseStatus().toString(), response.getResponseStatus()
                        .getReasonPhrase());
            }
        } finally {
            if (response != null) {
    ...
    public List<Tag> listTags() {
        ...
        log.debug("/api/3/action/tag_list: {}ms", System.currentTimeMillis() - start);
    ...
    public List<String> autoSuggestMetadata(String fragment) {
        ...
        log.trace("REST > calling util api 'dataset/autocomplete' with: {}", fragment);
        long start = System.currentTimeMillis();
        JsonNode result = util.getMetadataAutoComplete(fragment);
        log.debug("/api/2/util/dataset/autocomplete: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", result);
    ...
    public List<String> autoSuggestTags(String fragment) {
        ...
        log.trace("REST > calling util api 'tag/autocomplete' with: {}", fragment);
        long start = System.currentTimeMillis();
        JsonNode result = util.getTagsAutoComplete(fragment);
        log.debug("/api/2/util/tag/autocomplete: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", result);
    ...
    public List<String> autoSuggestFormats(String fragment) {
        ...
        log.trace("REST > calling util api 'format_autocomplete' with: {}", fragment);
        long start = System.currentTimeMillis();
        JsonNode result = util.getFormatsAutoComplete(fragment);
        log.debug("/api/2/util/resource/format_autocomplete: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", result);
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#queryMetadata(de.fhg.fokus.odp.registry .queries.Query)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#queryDatasets(de.fhg.fokus.odp.registry .queries.Query)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#queryApplications(de.fhg.fokus.odp .registry.queries.Query)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#queryDocuments(de.fhg.fokus.odp.registry .queries.Query)
     */
    @Override
    ...
    public Tag getTag(String name) {
        ...
        log.debug("/api/3/action/tag_show: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", result);
    ...
    public User findUser(String name) {
        ...
        ClientResponse<JsonNode> response = null;
        try {
            log.trace("REST > calling action api 'user_show' with: {}", userName);
            long start = System.currentTimeMillis();
            response = action.showUser(authorizationKey, userName);
            log.debug("/api/3/action/user_show: {}ms", System.currentTimeMillis() - start);
            ...
            if (response.getStatus() == Response.Status.OK.getStatusCode()) {
                ...
                log.trace("REST < (http) returns: {} - {}", response.getResponseStatus().name(), response.getResponseStatus()
                        .getReasonPhrase());
            }
        } finally {
            if (response != null) {
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#createUser(java.lang.String, java.lang.String, java.lang.String)
     */
    @Override
    public User createUser(String name, String email, String password) {
        ...
        try {
            ...
            log.debug("/api/3/action/user_create: {}ms", System.currentTimeMillis() - start);
            if (response.getStatus() == Response.Status.OK.getStatusCode()) {
    ...
    private <T> QueryResult<T> query(Query query, Class<T> clazz) {
        ...
        ClientResponse<JsonNode> response = null;
        try {
            log.info("REST > calling action api 'package_search' with: {}", param);
            long start = System.currentTimeMillis();
            response = action.metadataSearch(param);
            log.debug("/api/3/action/package_search: {}ms", System.currentTimeMillis() - start);
            ...
            if (response.getResponseStatus() == Status.OK) {
                ...
                log.debug("parsed and added {} metadata objects to the result list", all.size());
                ...
                if (facets != null && !facets.isEmpty()) {
                    ...
                    for (QueryFacetItem license : licenses.getItems()) {
                        for (Licence lic : licencesCache) {
                            if (license.getName().equalsIgnoreCase(lic.getName())) {
                                ((QueryFacetItemImpl) license).setDisplayName(lic.getTitle());
                            }
                        }
                    }
                ...
                log.debug("REST < (http) returns: {} - {}", response.getResponseStatus().getStatusCode(), response.getResponseStatus()
                        .getReasonPhrase());
                queryResult.setSuccess(false);
                queryResult.setErrorMessage(response.getResponseStatus().getReasonPhrase());
            }
        } finally {
            if (response != null) {
                response.releaseConnection();
    ...
    private JsonNode queryToJson(Query query) {
        ...
        case EXTENDED:
            if (!query.getTypes().isEmpty()) {
                ...
                searchterm = (searchterm == null) ? types.toString() : searchterm + " " + types.toString();
            }
            ...
            if (!query.getCategories().isEmpty()) {
                ...
                searchterm = (searchterm == null) ? categories.toString() : searchterm + " " + categories.toString();
            }
            ...
            if (!query.getTags().isEmpty()) {
                ...
                searchterm = (searchterm == null) ? tags.toString() : searchterm + " " + tags.toString();
            }
            ...
            if (!query.getLicences().isEmpty()) {
                ...
                searchterm = (searchterm == null) ? licences.toString() : searchterm + " " + licences.toString();
            }
            ...
            if (!query.getFormats().isEmpty()) {
                ...
                searchterm = (searchterm == null) ? formats.toString() : searchterm + " " + formats.toString();
            }

            if (query.getIsOpen() != null) {
                String isopen = "isopen:" + query.getIsOpen().toString();
                searchterm = (searchterm == null) ? isopen : searchterm + " " + isopen;
            }

            if (query.getCoverageFrom() != null) {
                String from = "+temporal_coverage_from:[" + dateFormat.format(query.getCoverageFrom()) + " TO *]";
                searchterm = (searchterm == null) ? from : searchterm + " " + from;
            }

            if (query.getCoverageTo() != null) {
                String to = "+temporal_coverage_to:[* TO " + dateFormat.format(query.getCoverageTo()) + "]";
        ...
        int offset = (query.getMax() * query.getPageoffset()) + query.getOffset();
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#createMetadata(de.fhg.fokus.odp.registry .model.MetadataEnumType)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#rateMetadata(de.fhg.fokus.odp.registry .model.User, java.lang.String, int)
     */
    @Override
    public void rateMetadata(User user, String metadata, int rate) {
        ...
        long start = System.currentTimeMillis();
        JsonNode node = action.createMetadataRating(((UserImpl) user).getApikey(), body);
        log.debug("/api/3/action/rating_create: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", node);
    }
     ...
     * (non-Javadoc)
     * 
     * @see de.fhg.fokus.odp.registry.ODRClient#showRoles(de.fhg.fokus.odp.registry .model.User, java.lang.String)
     */
    @Override
    public List<String> showRoles(User user, String object) {
        ...
        log.debug("/api/3/action/roles_show: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", node);
    ...
    public void updateRoles(User user, String object, List<String> roles) {
        ...
        log.debug("/api/3/action/user_role_update: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", node);
    }
    ...
    private boolean isSuccess(JsonNode result) {
        ...
        return (success != null && success.isBoolean()) ? success.getBooleanValue() : false;
    }
    ...
    @Override
    public void persistMetadata(User user, Metadata metadata) throws OpenDataRegistryException {
        MetadataImpl impl = (MetadataImpl) metadata;
        String auth = metadata.getType().equals(MetadataEnumType.APPLICATION) || user == null ? authorizationKey : ((UserImpl) user)
                .getApikey();
        ...
        if (impl.isNew()) {
            ...
            log.trace("REST > calling action api 'package_create' with: {}", node);
            long start = System.currentTimeMillis();
            JsonNode result = action.createMetadata(auth, node);
            log.debug("/api/3/action/package_create: {}ms", System.currentTimeMillis() - start);
            log.trace("REST < returns: {}", result);
        } else {
            JsonNode node = impl.write();
            ...
            log.trace("REST > calling action api 'package_update' with: {}", node);
            long start = System.currentTimeMillis();
            JsonNode result = action.updateMetadata(auth, node);
            log.debug("/api/3/action/package_update: {}ms", System.currentTimeMillis() - start);
            log.trace("REST < returns: {}", result);
        }
    }

    @Override
    public Metadata getMetadata(User user, String name) throws OpenDataRegistryException {
        ObjectNode body = OM.createObjectNode();
        body.put("id", name);
        String key = user != null ? ((UserImpl) user).getApikey() : authorizationKey;

        log.trace("REST > calling action api 'package_show' with: {}", body);
        long start = System.currentTimeMillis();
        JsonNode node = action.showMetadata(key, body);
        log.debug("/api/3/action/package_show: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", node);
    ...
    private MetadataImpl generateImpl(JsonNode node) throws OpenDataRegistryException {
    ...
    public String mungeTitleToName(String title) {
        log.trace("REST > calling util api 'munge_title_to_name' with: {}", title);
        long start = System.currentTimeMillis();
        String result = strip(util.mungeTitleToName(title));
        log.debug("/api/2/util/dataset/munge_title_to_name: {}ms", System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", result);
        return result;
    }
    ...
    public void loadRating(Metadata metadata) {
        ...
        log.trace("REST > calling model dataset api with: {}", metadata.getName());
        long start = System.currentTimeMillis();
        JsonNode node = model.getDataset(metadata.getName());
        log.debug("/api/2/rest/dataset/{}: {}ms", metadata.getName(), System.currentTimeMillis() - start);
        log.trace("REST < returns: {}", node);

        JsonNode count = node.get("ratings_count");
        if (count != null && count.isNumber()) {
            ((MetadataImpl) metadata).setRatingCount(count.getNumberValue().intValue());
        }
        JsonNode average = node.get("ratings_average");
        if (average != null && average.isDouble()) {
            ((MetadataImpl) metadata).setAverageRating(average.getNumberValue().intValue());
        }
    }
    ...
    public List<RelationshipBean> listRelationships(String name, String type) {
        ...
        ClientResponse<JsonNode> response = null;
        try {
            log.trace("REST > calling action api 'package_relationships_list' with: {}", body);
            long start = System.currentTimeMillis();
            response = action.listRelationships(authorizationKey, body);
            log.debug("/api/3/action/package_relationships_list: {}ms", System.currentTimeMillis() - start);
            if (response.getResponseStatus() == Status.OK) {
                JsonNode node = response.getEntity();
                log.trace("REST < returns: {}", node);
                ...
                if (isSuccess(node)) {
                    ...
                    for (JsonNode relationship : result) {
                        RelationshipBean bean = convert(relationship, RelationshipBean.class);
